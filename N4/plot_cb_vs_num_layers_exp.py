...
for num_meas_layers in num_meas_layers_list:
    with open(f'cb_fullgraph4_exp/N4_L{num_meas_layers}_sensor.pkl', 'rb') as file:
        sensor = pickle.load(file)
    cbs.append(cls_bound(sensor))
print(f"CBs = {cbs}")

# Load QB
with open(f'qb_fullgraph4/N4_L{num_prep_layers}_qb.pkl', 'rb') as file:
    qb = pickle.load(file)
print(f"QB = {qb}")
...
# Save figure
plt.tight_layout()
plt.savefig("_figures/N4_cb_vs_num_layers_exp.png", pad_inches=0.3, facecolor="white", bbox_inches="tight")
plt.savefig("_figures/N4_cb_vs_num_layers_exp.eps", pad_inches=0.3, facecolor="white", bbox_inches="tight")
